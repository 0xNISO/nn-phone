...
function IsControlsFree()
    local PlayerData = NNCore.Functions.GetPlayerData();
        ...
        if IsControlJustPressed(0, Config.OpenPhone) then
            if not PhoneData.isOpen then
                ...
                if not IsHandcuffed then
                    ...
                    NNCore.Functions.Notify("Action Impossible..", "error")
                end
            end
        end
        Citizen.Wait(3)
...
RegisterNetEvent('nn-phone:client:RaceNotify')
AddEventHandler('nn-phone:client:RaceNotify', function(message)
        SendNUIMessage({
...
RegisterNetEvent('nn-phone:client:AddRecentCall')
AddEventHandler('nn-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('nn-phone:server:SetPhoneAlerts', "phone")
...
RegisterNetEvent('NNCore:Client:OnJobUpdate')
AddEventHandler('NNCore:Client:OnJobUpdate', function(JobInfo)
    SendNUIMessage({
...
RegisterNUICallback('ClearRecentAlerts', function(data, cb)
    TriggerServerEvent('nn-phone:server:SetPhoneAlerts', "phone", 0)
...
RegisterNUICallback('SetBackground', function(data)
    ...
    TriggerServerEvent('nn-phone:server:SaveMetaData', PhoneData.MetaData)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            NNCore.Functions.TriggerCallback('nn-phone:server:GetPhoneData', function(pData)   
...
function LoadPhone()
    ...
    NNCore.Functions.TriggerCallback('nn-phone:server:GetPhoneData', function(pData)
        PlayerJob = NNCore.Functions.GetPlayerData().job
        PhoneData.PlayerData = NNCore.Functions.GetPlayerData()
...
RegisterNetEvent('NNCore:Client:OnPlayerUnload')
AddEventHandler('NNCore:Client:OnPlayerUnload', function()
    PhoneData = {
        MetaData = {},
        isOpen = false,
        PlayerData = nil,
...
RegisterNetEvent('NNCore:Client:OnPlayerLoaded')
AddEventHandler('NNCore:Client:OnPlayerLoaded', function()
    LoadPhone()
end)

RegisterNUICallback('HasPhone', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-phone:server:HasPhone', function(HasPhone)
        cb(HasPhone)
    end)
end)
...
function OpenPhone()
    NNCore.Functions.TriggerCallback('nn-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            -- NNCore.Functions.TriggerCallback('nn-phone:server:GetGarageVehicles', function(vehicles)
            --     PhoneData.GarageVehicles = vehicles
            -- end)
        else
            NNCore.Functions.Notify("You don't have a phone", "error")
        end
    end)
end
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('nn-phone:server:RemoveMail', MailId)
    cb('ok')
end)

RegisterNetEvent('nn-phone:client:UpdateMails')
AddEventHandler('nn-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = "UpdateMails",
        Mails = NewMails
    })
...
RegisterNUICallback('AcceptMailButton', function(data)
    ...
    TriggerServerEvent('nn-phone:server:ClearButtonData', data.mailId)
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('nn-phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    NNCore.Functions.TriggerCallback('nn-phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('nn-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('nn-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    NNCore.Functions.TriggerCallback('nn-phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
...
RegisterNetEvent('nn-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
	...
	table.insert(PhoneData.Chats, {
        ...
        if PhoneData.isOpen then
            ...
            Wait(100)
            NNCore.Functions.TriggerCallback('nn-phone:server:GetContactPictures', function(Chats)
                SendNUIMessage({
                    action = "UpdateChat",
	    ...
	    SendNUIMessage({
            ...
            TriggerServerEvent('nn-phone:server:SetPhoneAlerts', "whatsapp")
        end
    else
...
RegisterNetEvent("nn-phone-new:client:BankNotify", function(text)
    SendNUIMessage({
        action = "PhoneNotification",
        NotifyData = {
            title = "Bank", 
...
RegisterNetEvent('nn-phone:client:NewMailNotify', function(MailData)
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
                title = "Mail",
    ...
    TriggerServerEvent('nn-phone:server:SetPhoneAlerts', "mail")
end)

RegisterNUICallback('PostAdvert', function(data)
    TriggerServerEvent('nn-phone:server:AddAdvert', data.message)
end)

RegisterNetEvent('nn-phone:client:UpdateAdverts', function(Adverts, LastAd)
...
RegisterNetEvent('nn-phone:client:Adverts',function(Update)  --This function send the adverts to the phone
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('nn-phone:server:SetPhoneAlerts', "whatsapp", newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    NNCore.Functions.TriggerCallback('nn-phone:server:PayInvoice', function(CanPay, Invoices)
        if CanPay then PhoneData.Invoices = Invoices end
        cb(CanPay)
    end, society, amount, invoiceId, senderCitizenId)
    TriggerServerEvent('nn-phone:server:BillingEmail', data, true)
end)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    NNCore.Functions.TriggerCallback('nn-phone:server:DeclineInvoice', function(CanPay, Invoices)
        PhoneData.Invoices = Invoices
        cb('ok')
    end, society, amount, invoiceId)
    TriggerServerEvent('nn-phone:server:BillingEmail', data, false)
end)

RegisterNetEvent('nn-phone:client:BillingEmail')
AddEventHandler('nn-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('nn-phone:server:sendNewMail', {
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Citizen.Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('nn-phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
...
RegisterNetEvent('nn-phone:client:UpdateHashtags')
AddEventHandler('nn-phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNUICallback('PostNewTweet', function(data, cb)
    ...
    for i = 2, #Hashtag, 1 do
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            TriggerServerEvent('nn-phone:server:UpdateHashtags', Handle, TweetMessage)
        end
    end
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            if (Firstname ~= nil and Firstname ~= "") and (Lastname ~= nil and Lastname ~= "") then
                if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                    TriggerServerEvent('nn-phone:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                end
            end
        end
    end
    ...
    TriggerServerEvent('nn-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
end)

RegisterNetEvent('nn-phone:client:TransferMoney')
AddEventHandler('nn-phone:client:TransferMoney', function(amount, newmoney)
...
RegisterNetEvent('nn-phone:client:UpdateTweets')
AddEventHandler('nn-phone:client:UpdateTweets', function(src, Tweets, NewTweetData)
...
RegisterNetEvent('nn-phone:client:GetMentioned')
AddEventHandler('nn-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNUICallback('ClearMentions', function()
    ...
    TriggerServerEvent('nn-phone:server:SetPhoneAlerts', "twitter", 0)
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('nn-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    if tonumber(PhoneData.PlayerData.money.bank) >= data.amount then
        ...
        TriggerServerEvent('nn-phone:server:TransferMoney', data.iban, data.amount)
        local cbdata = {
            CanTransfer = true,
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        NNCore.Functions.TriggerCallback('nn-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('CallContact', function(data, cb)
    ...
    NNCore.Functions.TriggerCallback('nn-phone:server:GetCallState', function(CanCall, IsOnline, contactData)
        local status = { 
            CanCall = CanCall, 
            IsOnline = IsOnline,
...
CallContact = function(CallData, AnonymousCall)
    ...
    TriggerServerEvent('nn-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('nn-phone:server:SetCallState', true)
...
CancelCall = function()
    TriggerServerEvent('nn-phone:server:CancelCall', PhoneData.CallData)
    if PhoneData.CallData.CallType == "ongoing" then
        exports['nn-voice']:removePlayerFromCall(PhoneData.CallData.CallId)
    ...
    TriggerServerEvent('nn-phone:server:SetCallState', false)
...
RegisterNetEvent('nn-phone:client:CancelCall')
AddEventHandler('nn-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == "ongoing" then
        SendNUIMessage({
...
RegisterNetEvent('nn-phone:client:GetCalled')
AddEventHandler('nn-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('nn-phone:server:AddRecentCall', "missed", CallData)
                    break
                end
                Citizen.Wait(Config.RepeatTimeout)
            else
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('nn-phone:server:AnswerCall', PhoneData.CallData)
        exports['nn-voice']:addPlayerToCall(PhoneData.CallData.CallId)
...
RegisterNetEvent('nn-phone:client:AnswerCall')
AddEventHandler('nn-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = "ongoing"
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)

RegisterNUICallback('FetchVehicleResults', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then 
            for k, v in pairs(result) do
                NNCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    local vehicle = NNCore.Functions.GetClosestVehicle()
    local plate = GetVehicleNumberPlateText(vehicle)
    local vehname = GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)):lower()
    NNCore.Functions.TriggerCallback('nn-phone:server:ScanPlate', function(result)
        NNCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
	    if NNCore.Shared.Vehicles[vehname] ~= nil then
                result.label = NNCore.Shared.Vehicles[vehname]['name']
            else
...
RegisterNetEvent('nn-phone:client:addPoliceAlert')
AddEventHandler('nn-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = NNCore.Functions.GetPlayerData().job
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    NNCore.Functions.Notify('GPS Location set: '..data.alert.title)
...
function GetClosestPlayer()
    local closestPlayers = NNCore.Functions.GetPlayersFromCoords()
...
RegisterNetEvent('nn-phone:client:GiveContactDetails')
AddEventHandler('nn-phone:client:GiveContactDetails', function()
    local ped = PlayerPedId()

    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('nn-phone:server:GiveContactDetails', PlayerId)
    else
        NNCore.Functions.Notify("No one nearby!", "error")
    end
end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('nn-phone:server:RemoveContact', Name, Number)
end)

RegisterNetEvent('nn-phone:client:AddNewSuggestion')
AddEventHandler('nn-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications["phone"].Alerts = Config.PhoneApplications["phone"].Alerts + 1
    TriggerServerEvent('nn-phone:server:SetPhoneAlerts', "phone", Config.PhoneApplications["phone"].Alerts)
end)

RegisterNUICallback('GetCryptoData', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNetEvent('nn-phone:client:RemoveBankMoney')
AddEventHandler('nn-phone:client:RemoveBankMoney', function(amount)
    --if PhoneData.isOpen then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNetEvent('nn-phone:client:AddTransaction')
AddEventHandler('nn-phone:client:AddTransaction', function(SenderData, TransactionData, Message, Title)
    local Data = {
    ...
    TriggerServerEvent('nn-phone:server:AddTransaction', Data)
end)
...
RegisterNUICallback('GetAvailableRaces', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data)
    TriggerServerEvent('nn-lapraces:server:JoinRace', data.RaceData)
end)

RegisterNUICallback('LeaveRace', function(data)
    TriggerServerEvent('nn-lapraces:server:LeaveRace', data.RaceData)
end)

RegisterNUICallback('StartRace', function(data)
    TriggerServerEvent('nn-lapraces:server:StartRace', data.RaceData.RaceId)
end)

RegisterNetEvent('nn-phone:client:UpdateLapraces')
AddEventHandler('nn-phone:client:UpdateLapraces', function()
    SendNUIMessage({
...
RegisterNUICallback('GetRaces', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('nn-lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
end)

RegisterNUICallback('HasCreatedRace', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(data, cb)
    local InRace = exports['nn-lapraces']:IsInRace()
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        local data = {
            IsAuthorized = IsAuthorized,
            IsBusy = exports['nn-lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('nn-lapraces:server:CreateLapRace', data.TrackName)
end)

RegisterNUICallback('GetRacingLeaderboards', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-lapraces:server:GetRacingData', function(RaceData)
        local ped = PlayerPedId()
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('nn-lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
            NNCore.Functions.Notify('You\'re too far away from the race. GPS has been set to the race.', 'error', 5000)
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == "editor" then
        cb(exports['nn-lapraces']:IsInEditor())
    else
        cb(exports['nn-lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)

RegisterNUICallback('GetPlayerKeys', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-phone:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    NNCore.Functions.Notify("GPS has been set to " .. data.HouseData.HouseData.adress .. "!", "success")
end)

RegisterNUICallback('RemoveKeyholder', function(data)
    TriggerServerEvent('nn-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    NNCore.Functions.TriggerCallback('nn-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
    end, TransferedCid, data.HouseData)
end)

RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    NNCore.Functions.Notify('GPS has been set!', 'success')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(data, cb)
    NNCore.Functions.TriggerCallback('nn-phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('nn-phone:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('RemoveApplication', function(data, cb)
    TriggerServerEvent('nn-phone:server:RemoveInstallation', data.app)
end)

RegisterNetEvent('nn-phone:RefreshPhone')
AddEventHandler('nn-phone:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
...
RegisterNUICallback('GetTruckerData', function(data, cb)
    local TruckerMeta = NNCore.Functions.GetPlayerData().metadata["jobrep"]["trucker"]
    local TierData = exports['nn-trucker']:GetTier(TruckerMeta)
    cb(TierData)
end)
