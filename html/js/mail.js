...
$(document).on('click', '.mail', function(e){
    ...
    NN.Phone.Functions.SetupMail(MailData);
...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://nn-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://nn-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
NN.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(NN.Phone.Data.PlayerData.charinfo.firstname+"."+NN.Phone.Data.PlayerData.charinfo.lastname+"@wildstar.co.il");
...
NN.Phone.Functions.SetupMail = function(MailData) {
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        $.post('https://nn-phone/PostAdvert', JSON.stringify({
            message: Advert,
        }));
    } else {
        NN.Phone.Notifications.Add("fas fa-ad", "Advertisement", "You can\'t post an empty ad!", "#ff8f1a", 2000);
    }
});

NN.Phone.Functions.RefreshAdverts = function(Adverts) {
    $("#advert-header-name").html("@"+NN.Phone.Data.PlayerData.charinfo.firstname+""+NN.Phone.Data.PlayerData.charinfo.lastname+" | "+NN.Phone.Data.PlayerData.charinfo.phone);
