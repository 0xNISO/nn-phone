...
SetupMeosHome = function() {
    $("#meos-app-name").html("Welcome " + NN.Phone.Data.PlayerData.charinfo.firstname + " " + NN.Phone.Data.PlayerData.charinfo.lastname);
}
...
$(document).on('click', '.house-adress-location', function(e){
    ...
    $.post('https://nn-phone/SetGPSLocation', JSON.stringify({
...
$(document).on('click', '.appartment-adress-location', function(e){
    ...
    $.post('https://nn-phone/SetApartmentLocation', JSON.stringify({
...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    NN.Phone.Notifications.Add("fas fa-university", "MDT", "House number copied!", "#badc58", 1750);
...
$(document).on('click', '.confirm-search-person-test', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://nn-phone/FetchSearchResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
                ...
                NN.Phone.Notifications.Add("politie", "MDT", "There are no search results!");
                $(".person-search-results").html("");
            }
        });
    } else {
...
$(document).on('click', '.confirm-search-person-house', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://nn-phone/FetchPlayerHouses', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
...
$(document).on('click', '.confirm-search-vehicle', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://nn-phone/FetchVehicleResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".vehicle-search-results").html("");
...
$(document).on('click', '.scan-search-vehicle', function(e){
    e.preventDefault();
    $.post('https://nn-phone/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            $(".vehicle-search-results").html("");
            var APK = "Yes";
            if (!vehicle.status) {
            ...
            NN.Phone.Notifications.Add("politie", "MDT", "No vehicle nearby!");
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData.coords != undefined && alertData.coords != null) {
        $.post('https://nn-phone/SetAlertWaypoint', JSON.stringify({
            alert: alertData,
        }));
    } else {
        NN.Phone.Notifications.Add("politie", "MDT", "This alert doesn't have a GPS location!");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    ...
    NN.Phone.Notifications.Add("politie", "MDT", "All alerts have been deleted!");
});
