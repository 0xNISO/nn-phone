        ...
        if (CanOpenApp) {
            if (NN.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $("#mySerialNumber").text("nn-" + NN.Phone.Data.PlayerData.metadata["phonedata"].SerialNumber);
                    $('#id1').val($('#id1').val() + 'more text');
                } else if (PressedApplication == "twitter") {
                    $.post('https://nn-phone/GetMentionedTweets', JSON.stringify({}), function(MentionedTweets){
                        NN.Phone.Notifications.LoadMentionedTweets(MentionedTweets)
                    })
                    $.post('https://nn-phone/GetHashtags', JSON.stringify({}), function(Hashtags){
                        NN.Phone.Notifications.LoadHashtags(Hashtags)
                    })
                    if (NN.Phone.Data.IsOpen) {
                        $.post('https://nn-phone/GetTweets', JSON.stringify({}), function(Tweets){
                            NN.Phone.Notifications.LoadTweets(Tweets);
                        });
                    }
                } else if (PressedApplication == "bank") {
                    NN.Phone.Functions.DoBankOpen();
                    $.post('https://nn-phone/GetBankContacts', JSON.stringify({}), function(contacts){
                        NN.Phone.Functions.LoadContactsWithNumber(contacts);
                    });
                    $.post('https://nn-phone/GetInvoices', JSON.stringify({}), function(invoices){
                    ...
                    $.post('https://nn-phone/GetMissedCalls', JSON.stringify({}), function(recent){
                        NN.Phone.Functions.SetupRecentCalls(recent);
                    });
                    $.post('https://nn-phone/GetSuggestedContacts', JSON.stringify({}), function(suggested){
                        NN.Phone.Functions.SetupSuggestedContacts(suggested);
                    });
                    $.post('https://nn-phone/ClearGeneralAlerts', JSON.stringify({
                        app: "phone"
                    }));
                } else if (PressedApplication == "mail") {
                    $.post('https://nn-phone/GetMails', JSON.stringify({}), function(mails){
                    ...
                    $.post('https://nn-phone/LoadAdverts', JSON.stringify({}), function(Adverts){
                        NN.Phone.Functions.RefreshAdverts(Adverts);
                    })
                } else if (PressedApplication == "garage") {
                    $.post('https://nn-phone/SetupGarageVehicles', JSON.stringify({}), function(Vehicles){
                        SetupGarageVehicles(Vehicles);
                    })
                } else if (PressedApplication == "crypto") {
                    $.post('https://nn-phone/GetCryptoData', JSON.stringify({
                        crypto: "qbit",
                    }), function(CryptoData){
                        SetupCryptoData(CryptoData);
                    })

                    $.post('https://nn-phone/GetCryptoTransactions', JSON.stringify({}), function(data){
                    ...
                    $.post('https://nn-phone/GetPlayerHouses', JSON.stringify({}), function(Houses){
                        SetupPlayerHouses(Houses);
                    });
                    $.post('https://nn-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                    ...
                    $.post('https://nn-phone/SetupStoreApps', JSON.stringify({}), function(data){
                        SetupAppstore(data); 
                    });
                } else if (PressedApplication == "trucker") {
                    $.post('https://nn-phone/GetTruckerData', JSON.stringify({}), function(data){
...
$(document).on('click', '.mykeys-key', function(e){
    ...
    $.post('https://nn-phone/SetHouseLocation', JSON.stringify({
        HouseData: KeyData
    }))
});
...
NN.Phone.Notifications.Add = function(icon, title, text, color, timeout) {
    $.post('https://nn-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
