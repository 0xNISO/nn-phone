...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Wallet: "+CryptoData.Portfolio+" NNbit('s)");
...
$(document).on('click', '.cryptotab-general-action', function(e){
    ...
    NN.Phone.Functions.HeaderTextColor("black", 300);
    ActionTab = Tab;
});
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (NN.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('https://nn-phone/BuyCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    NN.Phone.Data.PlayerData.money.bank = parseInt(NN.Phone.Data.PlayerData.money.bank) - parseInt(Price);
                    NN.Phone.Notifications.Add("fas fa-university", "Bank", "&#36; "+Price+",- has been withdrawn from your balance!", "#badc58", 2500);
                } else {
                    NN.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough money..", "#badc58", 1500);
                }
            });
        } else {
            NN.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough money..", "#badc58", 1500);
        ...
        NN.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Fill out all fields!", "#badc58", 1500);
    }
});
...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://nn-phone/SellCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    NN.Phone.Data.PlayerData.money.bank = parseInt(NN.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    NN.Phone.Notifications.Add("fas fa-university", "Bank", "&#36; "+Price+",- has been added to your balance!", "#badc58", 2500);
                } else {
                    NN.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough NNBits..", "#badc58", 1500);
                }
            });
        } else {
            NN.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough NNBits..", "#badc58", 1500);
    ...
    if ((Coins !== "") && (WalletId !== "")) {
        if (CryptoData.Portfolio >= Coins) {
            if (WalletId !== CryptoData.WalletId) {
                $.post('https://nn-phone/TransferCrypto', JSON.stringify({
                    Coins: Coins,
                    WalletId: WalletId,
                }), function(CryptoData){
                    if (CryptoData == "notenough") {
                        ...
                        NN.Phone.Notifications.Add("fas fa-university", "Crypto", "this Wallet-ID doesn't exist!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        NN.Phone.Notifications.Add("fas fa-university", "Crypto", "You transferred "+Coins+",- to "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                NN.Phone.Notifications.Add("fas fa-university", "Crypto", "You can't transfer to yourself..", "#badc58", 2500);
            }
        } else {
            NN.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough NNBits..", "#badc58", 1500);
        }
