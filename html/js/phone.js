...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('https://nn-phone/ClearRecentAlerts');
...
NN.Phone.Functions.SetupRecentCalls = function(recentcalls) {
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    console.log(NN.Phone.Data.AnonymousCall)

    $.post('https://nn-phone/CallContact', JSON.stringify({
        ContactData: cData,
        Anonymous: NN.Phone.Data.AnonymousCall,
    }), function(status){
        if (cData.number !== NN.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (NN.Phone.Data.AnonymousCall) {
                            NN.Phone.Notifications.Add("fas fa-phone", "Phone", "You started a anonymous call!");
                        }
                        $(".phone-call-outgoing").css({"display":"block"});
                        $(".phone-call-incoming").css({"display":"none"});
                        $(".phone-call-ongoing").css({"display":"none"});
                        $(".phone-call-outgoing-caller").html(cData.name);
                        NN.Phone.Functions.HeaderTextColor("white", 400);
                        NN.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
                        setTimeout(function(){
                            $(".phone-app").css({"display":"none"});
                            NN.Phone.Animations.TopSlideDown('.phone-application-container', 400, 0);
                            NN.Phone.Functions.ToggleApp("phone-call", "block");
                        }, 450);
    
                        CallData.name = cData.name;
                        CallData.number = cData.number;
                    
                        NN.Phone.Data.currentApplication = "phone-call";
                    } else {
                        NN.Phone.Notifications.Add("fas fa-phone", "Phone", "You're already in a call!");
                    }
                } else {
                    NN.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is busy!");
                }
            } else {
                NN.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is not available!");
            }
        } else {
            NN.Phone.Notifications.Add("fas fa-phone", "Phone", "You can't call yourself!");
        }
    });
});
...
NN.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            var ContactElement = '<div class="phone-contact" data-contactid="'+i+'"><div class="phone-contact-firstletter" style="font-size: 25px;"><i class="fa fa-user-circle"></i></div><div class="phone-contact-name">'+contact.name+'</div><div class="phone-contact-actions"><i class="fas fa-sort-down"></i></div><div class="phone-contact-action-buttons"> <i class="fas fa-phone-volume" id="phone-start-call"></i> <i class="fab fa-whatsapp" id="new-chat-phone" style="font-size: 2.5vh;"></i> <i class="fas fa-user-edit" id="edit-contact"></i> </div></div>'
            if (contact.status) {
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== NN.Phone.Data.PlayerData.charinfo.phone) {
        $.post('https://nn-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
        ...
        setTimeout(function(){
            ...
            NN.Phone.Functions.ToggleApp("phone", "none");
            NN.Phone.Functions.ToggleApp("whatsapp", "block");
            NN.Phone.Data.currentApplication = "whatsapp";
        
            $.post('https://nn-phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
                NN.Phone.Functions.SetupChatMessages(chat, {
            ...
            $(".whatsapp-chats").animate({left: 1+"vh"},100, function(){
                $(".whatsapp-chats").css({"display":"none"});
            });
        }, 400)
    } else {
        NN.Phone.Notifications.Add("fa fa-phone-alt", "Phone", "You can't whatsapp yourself..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    NN.Phone.Animations.TopSlideDown(".phone-edit-contact", 200, 0);
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://nn-phone/EditContact', JSON.stringify({
            ...
            NN.Phone.Functions.LoadContacts(PhoneContacts);
        });
        NN.Phone.Animations.TopSlideUp(".phone-edit-contact", 250, -100);
        setTimeout(function(){
            $(".phone-edit-contact-number").val("");
            $(".phone-edit-contact-name").val("");
        }, 250)
    } else {
        NN.Phone.Notifications.Add("fas fa-exclamation-circle", "Edit Contact", "Fill out all fields!");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('https://nn-phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#phone-plus-icon', function(e){
    ...
    NN.Phone.Animations.TopSlideDown(".phone-add-contact", 200, 0);
});
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://nn-phone/AddNewContact', JSON.stringify({
        ...
        NN.Phone.Animations.TopSlideUp(".phone-add-contact", 250, -100);
        setTimeout(function(){
        ...
        if (SelectedSuggestion !== null) {
            $.post('https://nn-phone/RemoveSuggestion', JSON.stringify({
        ...
        NN.Phone.Notifications.Add("fas fa-exclamation-circle", "Add Contact", "Fill out all fields!");
    }
});
        ...
        if (cData.number !== NN.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    NN.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is in a call!");
                }
            } else {
                NN.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is not available!");
            }
        } else {
            NN.Phone.Notifications.Add("fas fa-phone", "Phone", "You can't call your own number!");
        }
    });
}

CancelOutgoingCall = function() {
    if (NN.Phone.Data.currentApplication == "phone-call") {
        NN.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
    ...
    if (!Canceled) {
        if (!NN.Phone.Data.CallActive) {
            ...
            setTimeout(function(){
                ...
                $(".phone-app").css({"display":"none"});
                NN.Phone.Functions.HeaderTextColor("white", 400);
                $("."+NN.Phone.Data.currentApplication+"-app").css({"display":"none"});
...
NN.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        CallData = cData;
...
$(document).on('click', '.phone-currentcall-container', function(e){
    ...
    NN.Phone.Functions.HeaderTextColor("white", 500);
    NN.Phone.Animations.TopSlideDown('.phone-application-container', 500, 0);
    NN.Phone.Animations.TopSlideDown('.phone-call-app', 500, 0);
    NN.Phone.Functions.ToggleApp("phone-call", "block");
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('https://nn-phone/AnswerCall');
});

NN.Phone.Functions.AnswerCall = function(CallData) {
    $(".phone-call-incoming").css({"display":"none"});
    $(".phone-call-outgoing").css({"display":"none"});
    $(".phone-call-ongoing").css({"display":"block"});
...
NN.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
